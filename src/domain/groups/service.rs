    ...
    GroupService<GR, ER, SR, UR>
{
    pub fn new(
        ...
        Self { group_repo, expense_repo, settlement_repo, user_repo, cache }
    }
    ...
    pub async fn create(&self, data: CreateGroupRequest, creator_id: Uuid) -> Result<GroupResponse, AppError> {
        ...
        let group = self.group_repo.create(&new_group).await?;

        // Automatically add creator as ADMIN
        let new_member = NewGroupMemberEntity { group_id: group.id, user_id: creator_id, role: GroupRole::ADMIN };
        self.group_repo.add_member(&new_member).await?;
    ...
    pub async fn join_by_invite_code(&self, code: &str, user_id: Uuid) -> Result<GroupResponse, AppError> {
        let group = self.group_repo.find_by_invite_code(code).await?;
        ...
        match self.group_repo.add_member(&new_member).await {
            Ok(_) => {
    ...
    pub async fn find_all_by_user(&self, user_id: Uuid) -> Result<Vec<GroupResponse>, AppError> {
        let groups = self.group_repo.find_all_by_user(user_id).await?;
    ...
    pub async fn find_by_id(&self, id: Uuid, current_user_id: Option<Uuid>) -> Result<GroupResponse, AppError> {
        if let Some(user_id) = current_user_id {
            self.ensure_membership(id, user_id).await?;
        }

        let group = self.group_repo.find_by_id(id).await?;
        let group = group.ok_or(AppError::Business(BusinessError::GroupNotFound))?;
    ...
    pub async fn get_group_summary(
        ...
        let (group_opt, expenses_with_shares, settlements) = tokio::try_join!(
            self.group_repo.find_by_id(group_id),
            self.expense_repo.find_by_group_with_shares(group_id),
            self.settlement_repo.find_by_group(group_id),
        )?;
        ...
        let engine_settlements: Vec<SettlementForEngine> = settlements
            ...
            .collect();
        let balances =
            DebtEngine::calculate_net_balances(&user_ids, &engine_expenses, &engine_settlements).await?;
    ...
    pub async fn add_member(
        ...
        let member = self
            .group_repo
            .add_member(&NewGroupMemberEntity {
                group_id,
                user_id: data.user_id,
                role: data.role.unwrap_or(GroupRole::MEMBER),
            })
            .await
            .map_err(|err| map_unique_violation(err, &[("group_members", "already joined")]))?;

        let user = self.user_repo.find_by_id(data.user_id).await?;
    ...
    pub async fn delete_group(&self, id: Uuid, current_user_id: Uuid) -> Result<(), AppError> {
        ...
        let result = self.group_repo.soft_delete(id).await?;
        if result.is_none() {
