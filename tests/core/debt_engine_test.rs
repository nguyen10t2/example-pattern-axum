...
#[test]
fn test_zero_balances_for_equal_split() {
    let u1 = Uuid::now_v7();
    let u2 = Uuid::now_v7();
    ...
    let balances = DebtEngine::calculate_net_balances(&user_ids, &expenses, &settlements);
...
#[test]
fn test_neutralizing_with_settlements() {
    let u1 = Uuid::now_v7();
    let u2 = Uuid::now_v7();
    let user_ids = vec![u1, u2];
...
fn test_empty_inputs() {
    let balances = DebtEngine::calculate_net_balances(&[], &[], &[]);
    assert_eq!(balances.len(), 0);
    let suggestions = DebtEngine::simplify_debts(&[]);
    assert_eq!(suggestions.len(), 0);
}

#[test]
fn test_complex_circular_debts_simplification() {
    let a = Uuid::now_v7();
    let b = Uuid::now_v7();
    let c = Uuid::now_v7();
    let d = Uuid::now_v7();
    ...
    let balances = DebtEngine::calculate_net_balances(&[a, b, c, d], &expenses, &[]);
